...
if False:
    ...
    for g in grps:
        ...
        for s in sDCM:
            ...
            nimpa.array2nii(
                tmpim.astype(np.float32),
                ndct['affine'],
                fout,
                trnsp = (ndct['transpose'].index(0), ndct['transpose'].index(1), ndct['transpose'].index(2)),
                flip = ndct['flip'])
            #------------------------------------
...
out_fe = centi.run(ffbps, fmris, atlases, tracer='new', outpath=opth/'output_fbp_e', used_saved=True)
