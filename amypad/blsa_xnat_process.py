...
sjson = nixnat.get_list(xc["sbj"] + "?format=json", cookie=xc["cookie"])
# ---------------------------------------
...
exps = nixnat.get_list(
    xc["sbj"] + "/" + sbj_lbl + "/experiments?format=json", cookie=xc["cookie"]
)
...
for i, e in enumerate(elbls):
    if "PET" in e:
        v = np.int16(re.search(r"(?<=PET)\d*", e).group(0))
        pets.append((v, i))
    elif "MR" in e:
        v = np.int16(re.search(r"(?<=MR)\d*", e).group(0))
        mris.append((v, i))
            ...
            if k[:3].isnumeric():
                ...
                if "fsum" in ptrm:
                    ...
                    imcom_dyn = icom_correction(
                        dyndct, Cnt={"LOG": 20}, com=imcom["com_abs"]
                    )
                else:
                ...
                fout_t1 = os.path.join(
                    rpth, k + "_V" + str(pet_vst) + "_resampled_T1w-BC.nii.gz"
                )
                fout_pr = os.path.join(
                    rpth, k + "_V" + str(pet_vst) + "_resampled_T1w-Par.nii.gz"
                )
                ...
                print(
                    ...
                    \r==========================================================
                    """.format(
                        fprcl_r, imcom["fim"], imcom_dyn["fim"]
                    )
                )
                ...
                nixnat.put_file(
                    res_url + "/files?content=PRCL", fprcl_r, cookie=xc["cookie"]
                )
